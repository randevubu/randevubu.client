...
export default function AppointmentsPage() {
  ...
  const handleCancelAppointment = async () => {
    ...
    try {
      ...
      const { extractApiError } = await import('@/src/lib/utils/errorExtractor');
                        ...
                        <div className="px-3 pb-3 border-t border-[var(--theme-border)]/50 bg-gradient-to-br from-[var(--theme-backgroundSecondary)]/10 to-transparent">
                          <div className="pt-3 space-y-3">
                            ...
                            {(!appointment.business.settings?.priceVisibility?.hideAllServicePrices || appointment.service.description) && (
                              <div className={`grid gap-3 ${!appointment.business.settings?.priceVisibility?.hideAllServicePrices && appointment.service.description
                    ...
                    <div className="flex items-center space-x-0.5 sm:space-x-1">
                      {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                        <button
                          ...
                          className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg text-xs sm:text-sm font-semibold transition-all duration-200 ${currentPage === page
