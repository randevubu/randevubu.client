...
export const banCustomerSchema = z.object({
  ...
  // If it's temporary, durationDays is required
  if (data.isTemporary && !data.durationDays) {
    return false;
  }
  // If it's permanent, durationDays should not be provided
  if (!data.isTemporary && data.durationDays) {
    return false;
  }
  return true;
}, {
  message: "Duration is required for temporary bans and should not be provided for permanent bans",
  path: ["durationDays"]
});
