  ...
  const refreshToken = async (silent = false): Promise<string | null> => {
    try {
      ...
      if (!response.success) {
        ...
        // Only log non-missing-token errors, and only if not silent
        if (!silent && !isTokenMissing) {
          console.error('Failed to refresh token:', response.error?.message);
        }
      ...
      // Network or parsing errors - only log if not silent
      if (!silent) {
  ...
  useEffect(() => {
    const initializeAuth = async () => {
      try {
        ...
        // Silent initialization - only log unexpected errors
