...
import { DiscountCodeInput } from '../../../src/components/features/DiscountCodeInput';
import { DiscountApplication } from '../../../src/types/discount';
import { Business } from '../../../src/types/business';
import { SubscriptionPlan, BusinessSubscription } from '../../../src/types/subscription';
    ...
    try {
      ...
      if (response.success) {
        ...
        if (discountApplication) {
          // You could show a toast message here about successful discount application
          console.log('Discount applied successfully:', discountApplication);
        }
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="p-4 sm:p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl border border-blue-200">
                  ...
                  <div className="text-2xl sm:text-3xl font-bold text-gray-900">
                    {discountApplication 
                      ? formatPrice(discountApplication.finalAmount, selectedPlan.currency)
                    ...
                    {discountApplication && (
                      <div className="text-sm text-green-600 font-medium mt-2">
                        Orijinal fiyat: <span className="line-through">{formatPrice(selectedPlan.price, selectedPlan.currency)}</span>
                        {' '}- İndirim: {formatPrice(discountApplication.discountAmount, selectedPlan.currency)}
                      </div>
                    )}
                  </div>
                </div>
