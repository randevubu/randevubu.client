...
import { useUsageTracking, useUsageCharts, useUsageLimits } from '../../../src/lib/hooks/useUsageTracking';
      ...
      <div className="space-y-2">
        <div className="flex justify-between text-xs sm:text-sm text-gray-600">
          ...
          <span className={`font-semibold ${
            ...
            {(limit - current).toLocaleString()}{unit} kaldı
          </span>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 pb-8">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 pt-4 sm:pt-6 lg:pt-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
          <UsageProgressBar
            current={currentMonth.smssSent}
            limit={planLimits.smsQuota}
            label="SMS Kullanımı"
            unit=" SMS"
            colorScheme="blue"
            icon={
          ...
          <UsageProgressBar
            current={currentMonth.staffMembersActive}
            limit={planLimits.maxStaffPerBusiness}
          ...
          <UsageProgressBar
            current={currentMonth.customersAdded}
          ...
          <UsageProgressBar
            current={currentMonth.servicesActive}
          ...
          <UsageProgressBar
            current={currentMonth.storageUsedMB}
            limit={planLimits.storageGB * 1024}
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <UsageProgressBar
              current={Math.floor(currentMonth.appointmentsCreated / 30)} // Daily average
              limit={planLimits.maxAppointmentsPerDay}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              {usageAlerts.smsQuotaAlert.isNearLimit && (
                <UsageAlert
                  ...
                  message={`SMS kotanızın %${usageAlerts.smsQuotaAlert.percentage?.toFixed(1)}'ine ulaştınız. ${usageAlerts.smsQuotaAlert.remaining} SMS hakkınız kaldı.`}
                />
              )}
              ...
              {usageAlerts.staffLimitAlert.isAtLimit && (
                <UsageAlert
                  ...
                  message={`Personel limitinize ulaştınız (${usageAlerts.staffLimitAlert.current}/${usageAlerts.staffLimitAlert.limit}). Yeni personel eklemek için planınızı yükseltmeniz gerekiyor.`}
                />
              )}
              ...
              {usageAlerts.customerLimitAlert.isNearLimit && (
                <UsageAlert
                  ...
                  message={`Müşteri limitinizin %${usageAlerts.customerLimitAlert.percentage?.toFixed(1)}'ine ulaştınız. ${usageAlerts.customerLimitAlert.limit! - usageAlerts.customerLimitAlert.current!} müşteri daha ekleyebilirsiniz.`}
                />
              )}

              {usageAlerts.storageLimitAlert.isNearLimit && (
                <UsageAlert
                  type="warning"
                  title="Depolama Alanı Doluyor"
                  message={`Depolama alanınızın %${usageAlerts.storageLimitAlert.percentage?.toFixed(1)}'ini kullandınız. ${formatStorage(usageAlerts.storageLimitAlert.limitMB! - usageAlerts.storageLimitAlert.usedMB!)} alan kaldı.`}
                />
              )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
            <div className="text-center p-3 sm:p-4 bg-blue-50 rounded-2xl border border-blue-200">
              <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-blue-600 mb-1">
                {currentMonth.smssSent.toLocaleString()}
              </div>
            ...
            <div className="text-center p-3 sm:p-4 bg-green-50 rounded-2xl border border-green-200">
              <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-green-600 mb-1">
                {currentMonth.appointmentsCreated.toLocaleString()}
              </div>
            ...
            <div className="text-center p-3 sm:p-4 bg-purple-50 rounded-2xl border border-purple-200">
              <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-purple-600 mb-1">
                {currentMonth.customersAdded.toLocaleString()}
              </div>
            ...
            <div className="text-center p-3 sm:p-4 bg-amber-50 rounded-2xl border border-amber-200">
              <div className="text-lg sm:text-2xl lg:text-3xl font-bold text-amber-600 mb-1">
                {formatStorage(currentMonth.storageUsedMB)}
              </div>
